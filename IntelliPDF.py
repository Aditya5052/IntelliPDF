...
def get_selected_word_meanings(event, pdf_viewer, result_text):
    ...
    if selected_words:
        ...
        popup_text.config(state=tk.NORMAL)  # Allow modification
...
def main():
    ...
    pdf_viewer.config(state=tk.NORMAL)  # Allow modification

    result_text = tk.Text(root, wrap=tk.WORD, height=10, width=50)
    result_text.config(state=tk.NORMAL)  # Allow modification
